					...
					<xsl:for-each select="$y-dimension">
						...
						<tbody>
							...
							<tr height="24" style="mso-height-source:userset;height:18.0pt">
								<td height="24" class="xl6622598" style="height:18.0pt">IVA POR PAGAR</td>
								<xsl:for-each select="$x-dimension">
									<xsl:variable name="amt" select="key('polizas',concat($rs,'::',.,'::','iva por pagar'))"/>
									<td class="xl6722598" align="right">
										<!--<xsl:apply-templates mode="xo-scope" select="$amt"/>-->
										<a class="link" href="?total={sum($amt)}#iva_por_pagar?@razon_social={$rs}&amp;@fecha={.}&amp;@clasificacion=28&amp;&amp;@view=polizas">
											<xsl:apply-templates mode="xo:scope" select="$amt"/>
							...
							<tr height="24" style="mso-height-source:userset;height:18.0pt">
								<td height="24" class="xl6622598" style="height:18.0pt">IVA POR PAGAR BANCOS</td>
								<xsl:for-each select="$x-dimension">
									<xsl:variable name="amt" select="key('polizas',concat($rs,'::',.,'::','iva por pagar bancos'))|key('polizas',concat($rs,'::',.,'::','activo a corto plazo'))"/>
									<td class="xl6722598" align="right">
										<!--<xsl:apply-templates mode="xo-scope" select="$amt"/>-->
										<a class="link" href="?total={sum($amt)}#iva_por_pagar?@razon_social={$rs}&amp;@fecha={.}&amp;@clasificacion=11,12&amp;@view=polizas">
											<xsl:apply-templates mode="xo:scope" select="$amt"/>
											<xsl:call-template name="format">
												<xsl:with-param name="value" select="sum($amt)"/>
											</xsl:call-template>
							...
							<tr height="24" style="mso-height-source:userset;height:18.0pt">
								...
								<xsl:for-each select="$x-dimension">
									<xsl:variable name="clientes" select="key('polizas',concat($rs,'::',.,'::','iva por pagar'))"/>
									<xsl:variable name="bancos" select="key('polizas',concat($rs,'::',.,'::','iva por pagar bancos'))|key('polizas',concat($rs,'::',.,'::','activo a corto plazo'))"/>
